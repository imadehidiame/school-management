...
export async function POST(req: NextRequest) {
  try {
      return new Promise<NextResponse>((resolve, reject) => {
          ...
          upload.single('file')(anyReq, mockRes as any, async (err) => {
            console.log("Multer middleware called");
            console.log("Req.body:", anyReq.body);
            console.log("Req.files:", anyReq.files);
            console.log("Req.file:", anyReq.file);

            if (err) { // Correct error handling
                console.error("Multer Error:", err);
                return resolve(NextResponse.json({ message: err.message }, { status: 400 }));
            }

            if (!anyReq.file) {
                console.log('Request file value:', anyReq); // Log the request if no file
                return resolve(NextResponse.json({ message: 'No file uploaded' }, { status: 400 }));
            }

            try {
                const response = await uploadToDrive(anyReq.file);
