...
export default function SectionModal(){
   // const {close,open} = useSchoolModalStore();
    ...
    return <Modal is_open={store.is_update_modal_open} on_close_action={store.close_update_modal} title={`Create/Update ${useBaseSchoolStore.getState().data?.section_naming}`} description={`Create a new ${base_store.data?.section_naming} or update an already existing one`}> 
